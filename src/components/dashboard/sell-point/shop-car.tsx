...
interface Props {
	currentClients: (typeof clients.$inferSelect)[];
}

export const ShopCar = ({ currentClients }: Props) => {
	const { shopCarProducts, client } = useShopCarStore((state) => ({
		shopCarProducts: state.products,
		client: state.client,
	}));

	const total = shopCarProducts.reduce(
		(acc, { price, quantity, discount }) => acc + price * quantity - discount,
		0,
	);

	const [currentSection, setCurrentSection] = useState<"client" | "ticket">(
		"ticket",
	);
			...
			<div className="flex mb-2">
				<button
					...
					className={`flex-1 p-2 text-white ${
						currentSection === "ticket" ? "bg-blue-500" : "bg-blue-400"
					}`}
				>
					Ticket
				</button>
				...
				<button
					...
					className={`flex-1 p-2 text-white ${
						currentSection === "client" ? "bg-blue-500" : "bg-blue-400"
					}`}
				>
					Clientes
				</button>
				...
				<div className="flex-1 h-full flex flex-col justify-between gap-2 animate-fade-in animate-duration-100">
					...
					<form
						onSubmit={(e) => {
							...
							if (
								shopCarProducts.some(({ stock, quantity }) => stock < quantity)
							) {
					...
					>
						<div className="flex w-full justify-between mb-2">
							...
							<div className="flex flex-col items-end justify-end">
								<span className="">Total: {priceFormmater.format(total)}</span>

								<span className="">
									Cambio:{" "}
									{priceFormmater.format(cash - total < 0 ? 0 : cash - total)}
								</span>
							</div>
						</div>
							...
							<div key={c.id} className="flex items-center gap-2 w-full">
								<button
									...
									className={`p-2 text-white flex-1 ${
										c.id === client ? "bg-blue-500" : "bg-blue-400"
									}`}
								...
								<a
									href={`/dashboard/clients/edit/${c.id}`}
								...
								<button
									...
									onClick={() => {
										fetch("/api/clients/delete", {
											...
											.then((res) => {
												if (res.ok) {
													useShopCarStore.setState({ client: 0 });
